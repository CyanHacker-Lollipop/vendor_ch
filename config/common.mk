...
PRODUCT_COPY_FILES += \
    vendor/ch/CHANGELOG.mkdn:system/etc/CHANGELOG-ch.txt ##Chnagelog-ch should be mentioned somewhere else as well

# Backup Tool
ifneq ($(WITH_GMS),true)
PRODUCT_COPY_FILES += \
    vendor/ch/prebuilt/common/bin/backuptool.sh:system/bin/backuptool.sh \
    vendor/ch/prebuilt/common/bin/backuptool.functions:system/bin/backuptool.functions \
...
PRODUCT_COPY_FILES += \
    vendor/ch/prebuilt/common/bin/otasigcheck.sh:system/bin/otasigcheck.sh
...
ifeq ($(MILESTONE),true)
	CH_VERSION := CyanHacker_v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-MILESTONE-$(shell date -u +%Y%m%d)
else	
	CH_VERSION := CyanHacker_v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)
endif
