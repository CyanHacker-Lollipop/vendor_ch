...
PRODUCT_COPY_FILES += \
    ...
    vendor/ch/prebuilt/common/apps/TricksterMod.apk:system/hackapp/FolderMount/fmount.apk \
    vendor/ch/prebuilt/common/apps/BetterBatteryStats.apk:system/priv-app/BetterBatteryStats/betterbatterystats.apk 
#we should try to build fmount only on devices with an external sd and remove it from settings if it isnt installed
#   a performance app
#   an explorer
#   themestore
...
ifeq ($(MILESTONE),true)
	...
	CH_DISPLAY_VERSION=$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-MILESTONE
else	
	CH_VERSION := CyanHacker-v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-NIGHTLY-$(shell date -u +%Y%m%d)
    CH_DISPLAY_VERSION=$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-NIGHTLY
endif
